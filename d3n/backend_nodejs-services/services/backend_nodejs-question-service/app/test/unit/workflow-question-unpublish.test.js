...
describe('WORKFLOW QUESTION UNPUBLISH', function () {
    this.timeout(20000);
    global.wsHelper.series().forEach(function (serie) {
        ...
        describe('[' + serie + '] ' + 'question unpublish basic tests', function () {
            QuestionWorkflow.removeAllInstances();
            ...
            it('[' + serie + '] ' + 'question workflow test instance removed on perform action', function (done) {
                ...
                */
                ...
                async.series([
                    ...
                    function (next) {
                        var content = {id: itemId};
                        global.wsHelper.apiSecureSucc(serie, 'question/questionDeactivate', tokenReviewer, content, null, function () {
                            setTimeout(function () {
                                try {
                                    assert.strictEqual(stubWFPerform.lastCall.args[0].getContent().taskId, 'QuestionUnpublish_' + itemId);
                                    assert.strictEqual(QuestionUnpublishWorkflow.hasInstance(itemId), false);
                                    next();
                                } catch (e) {
                                    next(e);
                                }
        ...
        describe('[' + serie + '] ' + 'question unpublish workflow', function () {
            ...
            it('[' + serie + '] ' + 'question unpublish workflow', function (done) {
                ...
                global.wsHelper.sinonSandbox.stub(QuestionApiFactory, "questionSetDeploymentStatus", function (params, callback) {
                ...
                */
                ...
                async.series([
                    ...
                    function (next) {
                        Question.update({ status: Question.constants().STATUS_DRAFT }, { where: { id: item.id }, individualHooks: true }).then(function (dbItem) {
                            return next();
                        }).catch(function (err) {
                            next(err);
                        });
                    },
                    // unpublish question, archive=false, itemStatus=NotPublished
                    function (next) {
                        wsPerformSpy.reset();
                        wsStartSpy.reset();
                        ...
                        global.wsHelper.apiSecureSucc(serie, 'question/questionDeactivate', tokenReviewer, content, null, function () {
                            setTimeout(function () {
                                try {
                                    ...
                                    assert.strictEqual(wsPerformSpy.lastCall.args[0].getContent().taskId, 'QuestionUnpublish_' + item.id);
                    ...
                    function (next) {
                        Question.update({ status: Question.constants().STATUS_ACTIVE }, { where: { id: item.id }, individualHooks: true }).then(function (dbItem) {
                            return next();
                        }).catch(function (err) {
                            next(err);
                        });
                    },
                    // unpublish question, archive=true, itemStatus=published
                    function (next) {
                        wsPerformSpy.reset();
                        wsStartSpy.reset();
                        
                        var content = {id: item.id, archive: true};
                        global.wsHelper.apiSecureSucc(serie, 'question/questionDeactivate', tokenReviewer, content, null, function () {
                            setTimeout(function () {
                                try {
                    ...
                    function (next) {
                        ...
                        QuestionApiFactory.questionSetDeploymentStatus.restore();
                        global.wsHelper.sinonSandbox.stub(QuestionApiFactory, "questionSetDeploymentStatus", function (params, callback) {
                    ...
                    function (next) {
                        var inst = QuestionUnpublishWorkflow.getInstance(item.id);
