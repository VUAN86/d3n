...
public class TombolaServiceClientMessageHandler extends DefaultJsonMessageHandler {
	...
	public TombolaServiceClientMessageHandler(TombolaManager tombolaManager, EventServiceClient eventServiceUtil,
	...
	public JsonMessageContent onUserMessage(RequestContext context) {
		...
		LOGGER.debug("onUserMessage message={}",message);
		...
		if (PaymentMessageTypes.isTransferFundsResponseWithTransactionId(message.getType(PaymentMessageTypes.class)) &&
			...
			LOGGER.debug("onUserMessage eventMessageType={}",eventMessageType);
...
//		}
	...
	private void onReceivedVoucherAssignResponseWithErrorForPrizePayout(PrizePayoutRequestInfo requestInfo,
		...
		dependencyServicesCommunicator.requestUserVoucherRelease(originalMessageDecoded, getSessionWrapper(),
				requestInfo.getVoucherId());
		tombolaManager.sendEmailToAdmin(requestInfo, errorMessage);
	}
	...
	private void onEventMessage(JsonMessage<? extends JsonMessageContent> message,
		...
		LOGGER.debug("onEventMessage message: {} eventMessageType: {}", message, eventMessageType);
	...
	private void onTombolaOpenCheckoutEvent(NotifySubscriberMessageContent messageContent) {
		...
		LOGGER.debug("onTombolaOpenCheckoutEvent getTombolaId={}",openCheckoutEvent.getTombolaId());
	...
	private void onTombolaDrawEvent(JsonMessage<NotifySubscriberMessageContent> message) {
		...
		tombolaDrawingEngine.tombolaDraw(drawEvent.getTombolaId(), drawEvent.getTenantId(), getSessionWrapper(),
				message);
	}
