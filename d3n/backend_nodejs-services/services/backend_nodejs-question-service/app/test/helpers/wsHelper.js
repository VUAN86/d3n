...
var FakeWorkflowService = require('./../services/workflow.fakeService.js');
var FakeProfileManagerService = require('./../services/profileManager.fakeService.js');
...
var WsHelper = function (series) {
    if (process.env.FAKE_SERVICES === 'true') {
        ...
        this._workflowFake = new FakeWorkflowService();
        this._profileManagerFake = new FakeProfileManagerService();
    }
...
WsHelper.prototype.build = function (done) {
    ...
    try {
        async.series([
            ...
            function (next) {
                if (process.env.FAKE_SERVICES === 'true') {
                    self._workflowFake.build(function (err) {
                        return next(err);
                    });
                } else {
                    return next();
            ...
            function (next) {
                if (process.env.FAKE_SERVICES === 'true') {
                    self._profileManagerFake.build(function (err) {
                        return next(err);
                    });
                } else {
                    return next();
        ...
        async.series([
            ...
            function (next) {
                if (process.env.FAKE_SERVICES === 'true') {
                    ...
                    self._workflowFake.shutdown();
                    self._srFake.shutdown();
                    self._profileManagerFake.shutdown();
                }
                return next();
            },
            function (next) {
