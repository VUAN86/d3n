...
public class GameEngineImpl implements GameEngine
{
	...
	@Inject
	public GameEngineImpl(JsonMessageUtil jsonUtil, GameAerospikeDao gameDAO, GameInstanceAerospikeDao gameInstanceDao, QuestionSelector questionSelector,
						  HealthCheckManager healthCheckManager, GameHistoryManager gameHistoryManagerImpl, PreselectedQuestionsDao preselectedQuestionsDao,
						  ...
						  ActiveGameInstanceDao activeGameInstanceDao, MessageCoordinator messageCoordinator, UserGameAccessService gameChecker, GameHistoryDao gameHistoryDAO) {
		...
		this.registerMessagePermissions = gameEngineMessageSchemaMapper.getMessagePermissions(GameEngineMessageTypes.REGISTER.getMessageName());
	...
	@Override
	public String registerForPaidQuiz24(ClientInfo clientInfo, String gameId, String entryFeeTransactionId, BigDecimal entryFeeAmount,
	...
	private SinglePlayerGameParameters getValidSinglePlayerGameConfig(SinglePlayerGameParameters singlePlayerGameConfig, Game game, boolean freeGame) {
		final SinglePlayerGameParameters validSinglePlayerGameConfig = Optional.ofNullable(singlePlayerGameConfig).orElse(new SinglePlayerGameParameters());
	...
	private String registerForQuiz24(ClientInfo clientInfo, Game game, SinglePlayerGameParameters singlePlayerGameConfig, String entryFeeTransactionId,
									 BigDecimal entryFeeAmount, Currency entryFeeCurrency) {
		...
		final GameInstanceBuilder gameInstanceBuilder = GameInstanceBuilder.create(clientInfo, game).withEntryFeeTransactionId(entryFeeTransactionId)
	...
	@Override
	public String registerForMultiplayerGame(ClientInfo clientInfo, String mgiId, String entryFeeTransactionId, BigDecimal entryFeeAmount,
		...
		final GameInstanceBuilder gameInstanceBuilder = GameInstanceBuilder.create(clientInfo, game).withMgiId(mgiId)
																		   .withEntryFee(entryFeeAmount, entryFeeCurrency)
		...
		multiplayerGameManager.registerForGame(mgiId, clientInfo, gameInstanceId, multiplayerGameConfig);
	...
	public GameInstance startGame(String userId, String gameInstanceId, String userLanguage) throws F4MInsufficientRightsException {
		...
		if (gameInstance != null) {
			...
			final CustomGameConfig customGameConfig = gameInstance.getMgiId() != null ? multiplayerGameManager.getMultiplayerGameConfig(gameInstance
																																				.getMgiId()) : null;
	...
	@Override
	public void validateSingleGameForRegistration(ClientInfo clientInfo, Game game, SinglePlayerGameParameters singlePlayerGameConfig) {
	...
	@Override
	public void endOfTheTippTournament(String mgiId, List<MultiplayerUserGameInstance> gameInstances) {
		try {
			for (MultiplayerUserGameInstance multiplayerUserGameInstance : gameInstances) {
				addCorrectAnswerToGameInstance(multiplayerUserGameInstance.getGameInstanceId());
			}
		} catch (Exception e) {
	...
	private void addCorrectAnswerToGameInstance(String gameInstanceId) {
		...
		final List<Question> questions = getAnswers(gameInstance.getUserId(), gameInstance, game, questionPools, numberOfQuestions, skipsAvailable);
		...
		final GameInstance updatedGameInstance = gameInstanceDao.update(gameInstanceId, new UpdateGameInstanceAction(jsonUtil)
		{
			...
			public String updateGameInstance(GameInstance gameInstance) {
				...
				gameState.endGame(gameInstance);
	...
	private List<Question> getAnswers(String userId, GameInstance gameInstance, final Game game, String[] questionPools, int numberOfQuestions,
									  int skipsAvailable) {
		final List<Question> questions;
		if (game.isTypeOf(GameType.TIPP_TOURNAMENT)) {//different set of questions
			questions = questionSelector.getAnswers(gameInstance, userId, game, gameInstance
					.getUserLanguage(), questionPools, numberOfQuestions, skipsAvailable);
		} else {
	...
	protected GameInstance startGameInstance(String gameInstanceId, String userLanguage, final Double userHandicap) {
		...
		final int skipsAvailable = game.getTotalSkipsAvailable();
		final int numberOfQuestions = gameInstance.getNumberOfQuestions() + skipsAvailable; // Have to select more questions to support skips
		final List<Question> questions = getQuestions(gameInstance.getUserId(), gameInstance
				.getMgiId(), game, userLanguage, questionPools, numberOfQuestions, skipsAvailable);
	...
	private List<Question> getQuestions(String userId, final String mgiId, final Game game, String userLanguage, String[] questionPools, int numberOfQuestions,
										int skipsAvailable) {
		final List<Question> questions;
		if (game.isTypeOf(GameType.USER_TOURNAMENT, GameType.PLAYOFF_TOURNAMENT, GameType.TOURNAMENT, GameType.QUIZ24, GameType.TIPP_TOURNAMENT)) {//different set of questions
	...
	private List<Question> preselectQuestions(String userId, Game game, String userLanguage, String[] questionPools, int numberOfQuestions, int skipsAvailable,
											  String mgiId) {
		...
		if (questionIndexesByMgi == null) {
			if (!game.hasInternationalQuestions()) { // only single language configured for game
				...
				questionIndexesByMgi = questions.stream().map(q -> q.getMultiIndex()).collect(Collectors.toList());
			} else {
	...
	private List<QuestionIndex> saveOrGetExistingQuestionIndexes(String userId, String mgiId, List<QuestionIndex> questionIndexesByMgi) {
	...
	private Game getGame(String gameId) {
		return gameId != null ? gameDAO.getGame(gameId) : null;
	}
			...
			private void nextStepOrQuestion(GameInstance gameInstance, boolean skipped) {
				...
				if (skipped || !hasNextStep) {
					if (state.hasNextQuestion(numberOfQuestions)) {
						...
						LOGGER.error("Attempted to perform next step on gameInstance[{}] with current question step [{}], status[{}] and numberOfQuestions [{}]", gameInstance
								.getId(), gameInstance.getGameState().getCurrentQuestionStep(), gameInstance.getGameState().getGameStatus(), numberOfQuestions);
	...
	@Override
	public void performRequestGetJackpotBalance(CancelTournamentGame content, SessionWrapper sessionWrapper, JsonMessage<?> sourceMessage) {
		messageCoordinator.performRequestGetJackpotBalance(gameDAO.getMgiId(content.getGameId()), sessionWrapper, sourceMessage);
	}

	public GameInstance calculateIfUnfinishedTippTournament(String gameInstanceId) throws F4MGameFlowViolation
	{
		return gameInstanceDao.calculateIfUnfinished(gameInstanceId, CloseUpReason.EXPIRED, null, null, false);
	}


	@Override
	public void performCancelTournamentGame(RequestContext context) {
		CancelTournamentGame cancelTournamentGame;
		if (context.getOriginalRequestInfo().getSourceMessage().getName().equals("event/notifySubscriber")) {
			NotifySubscriberMessageContent notificationContent = (NotifySubscriberMessageContent) context.getMessage().getContent();
			FinishTippTournament content = jsonUtil.fromJson(notificationContent.getNotificationContent().toString(), FinishTippTournament.class);
			Asserts.notNull(content, "FinishTippTournament must be not null.");
			cancelTournamentGame = new CancelTournamentGame(content.getGameId());
		} else {
			cancelTournamentGame = (CancelTournamentGame) context.getOriginalRequestInfo().getSourceMessage().getContent();
		}
		Asserts.notNull(cancelTournamentGame, "CancelTournamentGame must be not null.");
		String mgiId = gameDAO.getMgiId(cancelTournamentGame.getGameId());
		messageCoordinator.performCancelTournamentGame(multiplayerGameManager.getMultiplayerGameInstances(mgiId), cancelTournamentGame, mgiId, context
				.getOriginalRequestInfo());
	}

	@Override
	public void deleteMgiIdFromPublicGameList(String gameId) {
		String mgiId = gameDAO.getMgiId(gameId);
		multiplayerGameManager.markTournamentAsEnded(mgiId);
	}
	 ...
	 */
	@Override
	public GameInstance answerQuestion(String clientId, String gameInstanceId, int question, Long clientMsT, long receiveTimeStamp, String[] answers) {
		...
		final GameInstance updatedGameInstance = gameInstanceDao.update(gameInstanceId, new UpdateGameInstanceAction(jsonUtil)
		{
			...
			public String updateGameInstance(GameInstance gameInstance) {
				...
				if (currentQuestionStep != null && currentQuestionStep.getQuestion() == question) {
					...
					throw new F4MUnexpectedGameQuestionAnswered("Unexpected question answered for game instance[" + gameInstanceId + "] question[" + question + "], current state is ["
	...
	public long getServerMsT(String gameInstanceId, int question) {
		if (gameInstanceId != null) {
			GameInstance gameInstance = gameInstanceDao.getGameInstance(gameInstanceId);
			final GameState gameState = gameInstance.getGameState();
			long[] ServerMsT = gameState.getServerMsT(question);
			long sum = 0;
			for (Long serMsT : ServerMsT) {
				sum += serMsT;
			}
			return sum;
		} else return 0;
	...
	@Override
	public GameInstance registerStepSwitch(String clientId, String gameInstanceId, Long clientMsT, long receiveTimeStamp) {
		final Long propagationDealy = healthCheckManager.getGameInstancePropagationDelay(clientId);
		final GameInstance updatedGameInstance = gameInstanceDao.update(gameInstanceId, new UpdateGameInstanceAction(jsonUtil)
		{
			@Override
		...
		if (gameEndStatus != GameEndStatus.CALCULATING_RESULT && gameEndStatus != GameEndStatus.CALCULATED_RESULT && gameEndStatus != GameEndStatus.TERMINATED) {
			updateGameState(gameInstanceId, updatedGameInstance.getGameState());
		}
		return updatedGameInstance;
	}
	...
	private void validateIfHasPaidEntreeFee(Game game, CustomGameConfig customGameConfig, GameState gameState, String userId) {
		...
		Validate.notNull(gameState, "Game entry fee transaction id presence can be validated only via GameState");

		boolean canPlay = userGameAccessService.canPlay(game, userId);

		if (!GameUtil.isFreeGame(game, customGameConfig) && !gameState.hasEntryFeeTransactionId() && !GameUtil.haveMultipleGamesPurchase(game) && !canPlay) {
	...
	public void closeUpGameIntanceWithFailedProcessing(ClientInfo clientInfo, String gameInstanceId, String errorMessage) {
		gameHistoryManagerImpl.calculateIfUnfinished(clientInfo, gameInstanceId, CloseUpReason.BACKEND_FAILS, errorMessage, RefundReason.BACKEND_FAILED);
		activeGameInstanceDao.delete(gameInstanceId);
	}
	...
	public boolean initiateJokerPurchase(JokerType jokerType, SessionWrapper sessionWrapper,
		...
		if (jokerInformation.getPrice() == null || jokerInformation.getCurrency() == null || jokerInformation.isAlreadyPurchasedForCurrentQuestion()) {
			...
			messageCoordinator.requestTransferJokerPurchaseFee(gameInstance.getGame(), jokerInformation.getPrice(), jokerInformation
					.getCurrency(), sessionWrapper, message);
			return false;
		}
	}
	...
	private JokerInformation getJokerInformation(JokerType type, GameInstance gameInstance, int questionIndex) {
		...
		if (configuration != null && configuration.isEnabled()) {
			...
			boolean alreadyPurchasedForCurrentQuestion = jokersUsedOnQuestions != null && jokersUsedOnQuestions.contains(questionIndex);
			...
			if (configuration.getAvailableCount() == null) {
				// Unlimited
				return new JokerInformation(null, jokerAvailableForCurrentQuestion, alreadyPurchasedForCurrentQuestion, configuration
						.getCurrency(), configuration.getPrice());
			} else {
				// Limited
				...
				return new JokerInformation(availableCount, availableCount > 0 && jokerAvailableForCurrentQuestion, alreadyPurchasedForCurrentQuestion, configuration
						.getCurrency(), configuration.getPrice());
			}
		}
		...
		return gameInstanceDao.update(gameInstanceId, new UpdateGameInstanceAction(jsonUtil)
		{
			...
			public String updateGameInstance(GameInstance gameInstance) {
				...
				if (currentQuestionStep == null || currentQuestionStep.getQuestion() != questionIndex) {
					throw new F4MUnexpectedGameQuestionAnswered("Unexpected question skipped for game instance [" + gameInstanceId + "] question [" + questionIndex + "], current state is [" + (currentQuestionStep != null ? currentQuestionStep
							.getQuestion() : null) + "]");
	...
	@Override
	public PlayerGameHistoryResponse getPlayerGameHistory(PlayerGameHistoryRequest request) {
		 List<GameHistory> records = gameHistoryDAO.getGameHistory(request.getUserId());
		List <UserGameHistoryItem> gamePreHistory = new ArrayList<>();
		int requestedLimit = request.getLimit() == 0 ? 32767 : request.getLimit();
		for (GameHistory record : records){
			GameInstance instance = gameInstanceDao.getGameInstance(record.getGameInstanceId());
			UserGameHistoryItem item = new UserGameHistoryItem();
			item.setDate(record.getEndDateTime());
			item.setGameInstanceId(record.getGameInstanceId());
			item.setMultiplayerGameInstanceId(record.getMgiId());
			item.setStatus(record.getStatus().name());
			item.setGameId(record.getGameId());
			item.setGameTitle(instance.getGame().getTitle());
			item.setAppId(instance.getAppId());
			item.setOrderBy(request.getOrderBy().get(0));
			gamePreHistory.add(item);
		 }

		Collections.sort(gamePreHistory);
		List <UserGameHistoryItem> gameHistory = new ArrayList<>();
		int lastElement = (int) request.getOffset()+ requestedLimit;
		for (int recordInd = 0 + (int) request.getOffset(); ( recordInd < lastElement ) && recordInd < gamePreHistory.size(); recordInd++) {
			gameHistory.add(gamePreHistory.get(recordInd));
		}
		 return new PlayerGameHistoryResponse(gameHistory,  request.getLimit(),  request.getOffset(), gamePreHistory.size());
	}
}
