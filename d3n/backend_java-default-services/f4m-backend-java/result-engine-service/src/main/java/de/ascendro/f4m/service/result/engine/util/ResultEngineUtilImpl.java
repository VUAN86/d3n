...
public class ResultEngineUtilImpl implements ResultEngineUtil {
    ...
    public Results getResults(String gameInstanceId) throws F4MEntryNotFoundException {
        ...
        if (results != null) {
            ...
            results.addOriginalOrderAnswersToQuestionInfo(gameInstanceDao.getGameInstance(gameInstanceId));
        }
        return results;
    }
...
//         Add bonus points for all correct answers
        if (getCorrectAnswerPointCalculationType(resultConfig) != CorrectAnswerPointCalculationType.DONT_USE
                && results.getCorrectAnswerCount() == results.getTotalQuestionCount())
    ...
    public Results calculateResults(GameInstance gameInstance, ClientInfo clientInfo) {
        ...
        if (!gameInstance.getGameState().isCancelled() && !(game.getType() == GameType.TIPP_TOURNAMENT)) {
        ...
        LOGGER.debug("calculateResults results 1 {} ", results);
        ...
        LOGGER.debug("calculateResults results 2 {} ", results);
    ...
    private void calculateCorrectlyAnsweredQuestionsAndAnswerTime(Results results, GameInstance gameInstance, Set<Integer> skippedQuestions, boolean pointCalculator) {
        ...
        for (int i = 0; i < answers.length; i++) {
            ...
            LOGGER.debug("calculateCorrectlyAnsweredQuestionsAndAnswerTime 1 skippedQuestions {} ", skippedQuestions);

            if (skippedQuestions.contains(i)) {
                LOGGER.debug("calculateCorrectlyAnsweredQuestionsAndAnswerTime 2 skippedQuestions {} ", skippedQuestions);
                answerResults = results.addSkippedAnswer(i, question);
            } else {
                Answer answer = answers[i];
                LOGGER.debug("calculateCorrectlyAnsweredQuestionsAndAnswerTime 3 question {} , answer {} ", question, answer);
    ...
    private boolean isAnsweredCorrectly(Question question, Answer answer) {
        ...
        LOGGER.debug("isAnsweredCorrectly 1 maxMsT {} , serverMsT {}",maxMsT, serverMsT);
        ...
        if (ArrayUtils.isNotEmpty(serverMsT)) {
            ...
            for (int i = 0; i < maxMsT.length; i++) {
                ...
                LOGGER.debug("isAnsweredCorrectly 2 maxServerMsT {} ", maxServerMsT);
        ...
        LOGGER.debug("isAnsweredCorrectly 3 correct {} , provided {}",question.getCorrectAnswers(), answer.getAnswers());
    ...
    private int   calculatePerAnswerGamePoints(AnswerResults results, Question question, Answer answer, ResultConfiguration resultConfig) {
        ...
        if (calculationType != CorrectAnswerPointCalculationType.DONT_USE) {
            ...
            LOGGER.debug("calculationType    {}   {}   {} ",
                         calculationType != CorrectAnswerPointCalculationType.BASED_ON_QUESTION_COMMPLEXITY,
                         calculationType, CorrectAnswerPointCalculationType.BASED_ON_QUESTION_COMMPLEXITY);
            LOGGER.debug("calculationType    {}   {}   {} ",
                         resultConfig != null,
                         Objects.requireNonNull(resultConfig).getCorrectAnswerQuestionComplexityGamePoints() != null,
                         resultConfig.getCorrectAnswerQuestionComplexityGamePoints());
    ...
    private void calculateNewHandicap(GameInstance gameInstance, Results results, double handicapPoints, double oldHandicap, int maxGamePoints, boolean trainingMode) {
        ...
        if (gameInstance.getGame().isTournament()) {
            double handicap = (newHandicap - oldHandicap);
            double beforeGameHandicap = multiplayerGameInstanceDao.getUserHandicap(results.getMultiplayerGameInstanceId(), results.getUserId());
            multiplayerGameInstanceDao.recordUserHandicap(results.getMultiplayerGameInstanceId(), results.getUserId(), String.valueOf(handicap));
            newHandicap = newHandicap - beforeGameHandicap;
        }
    ...
    public UserResultsByHandicapRange calculateMultiplayerGameOutcome(String multiplayerGameInstanceId) {
        ...
        calculateJackpotPayout(resultsByHandicapRange, game);
     ...
     * Calculate jackpot payout.
     */
    private void calculateJackpotPayout(UserResultsByHandicapRange resultsByHandicapRange, Game game) {
        ResultConfiguration resultConfig = game.getResultConfiguration();
        LOGGER.debug("calculateJackpotPayout game {} resultConfig {}  multiplayer {} ", game, resultConfig, resultsByHandicapRange
                .getMultiplayerGameInstanceId());
        ...
        if (game.isDuel() && !GameUtil.isFreeGame(game, customGameConfig)) {
            if (resultsByHandicapRange.getTotalEntryFeeAmount() == null || resultsByHandicapRange.getEntryFeeCurrency() == null) {
                throw new F4MFatalErrorException(String.format("Game [%s] in muliplayer [%s] marked as paid duel, but does not have entry fee defined", game
                        .getGameId(), resultsByHandicapRange.getMultiplayerGameInstanceId()));
            }
            ...
            if (resultConfig.isJackpotCalculateByEntryFee()) {
                ...
                if (entryFeeCurrency == Currency.MONEY) {
                    if (totalEntryFeeAmount != null) {
                        jackpotAmount = totalEntryFeeAmount.multiply(config.getPropertyAsBigDecimal(ResultEngineConfig.JACKPOT_PAYOUT_PERCENT));
                        if (minimumJackpotAmount != null && jackpotAmount.compareTo(minimumJackpotAmount) <= 0) {
            ...
            if (jackpotAmount == null || jackpotCurrency == null) {
                throw new F4MFatalErrorException("Game " + game
                        .getGameId() + " marked as jackpot tournament, but no jackpot amount or currency could be determined");
            }
            ...
            if (handicapRangeCount > 1) {
                calculatedJackpotAmount = jackpotAmount.divide(BigDecimal.valueOf(handicapRangeCount), RoundingMode.DOWN);
            } else {
            ...
            if (multiplayerGameInstanceDao.getGameInstancesCount(resultsByHandicapRange.getMultiplayerGameInstanceId()) == 1) {
                resultsByHandicapRange.forEachRange(range -> calculateTournamentPayout(resultsByHandicapRange, range, game.getEntryFeeAmount(), game
                        .getEntryFeeCurrency()));
                ...
                resultsByHandicapRange
                        .forEachRange(range -> calculateTournamentPayout(resultsByHandicapRange, range, calculatedJackpotAmount, calculatedJackpotCurrency));
            }
        }
    }
    ...
    private void calculateTournamentPayout(UserResultsByHandicapRange multiplayerResults, HandicapRange range, BigDecimal jackpotAmount, Currency jackpotCurrency) {
        ...
        if (!placements.isEmpty()) {
            ...
            double factorBase = placements.stream()
                                          ...
                                          .sum();
            //   FIXME The problem with the c alculation of winnings. Remains surpluses.
            //   the calculation of the payout for each winner.
            placements.stream()
                      .limit(limitPlace(placements))
                      .forEach(userPlacement -> calculateTournamentPayout(userPlacement, factorBase, jackpotAmount, jackpotCurrency, logNormDist));
            LOGGER.debug("calculateTournamentPayout placements {} ", placements);
            if (jackpotCurrency == Currency.MONEY) {
                //   FIXME The problem with the calculation of winnings. Remains surpluses.
                checkWin(placements, jackpotAmount);
            }
        }
    }
    ...
    /**
     *  Verification of the calculation of the winning.
     * @param placements  list of players with prizes.
     * @param jackpotAmount jackpot multiPlayer.
     */
    private void checkWin(List<UserResults> placements, BigDecimal jackpotAmount) {
        double sum = (placements.stream()
                                .mapToDouble(a -> a != null && a.getJackpotWinning() != null ? a.getJackpotWinning().doubleValue() : 0.00)
                                .sum() - jackpotAmount.doubleValue());
        LOGGER.debug("checkWin sum {} ", sum);
        for (int i = 0; i < placements.size() - 1; i++ ,sum = sum - 0.01) {
            if (sum > 0.00) {
                BigDecimal win = placements.get(i).getJackpotWinning();
                Currency currency = placements.get(i).getJackpotWinningCurrency();
                LOGGER.debug("checkWin sum {}  win {}  currency {}  ", sum, win, currency);
                placements.get(i).setJackpotWinning((win.add(BigDecimal.valueOf(0.01))), currency);
            }else break;
        }
    }
    ...
    private void calculateTournamentPayout(UserResults result, double factorBase, BigDecimal jackpotAmount, Currency jackpotCurrency, LogNormalDistribution logNormDist) {
        if (result.isGameFinished()) {
            ...
            if (jackpotCurrency == Currency.MONEY) {
                BigDecimal win = BigDecimal.valueOf(jackpot).setScale(2, RoundingMode.DOWN);
                result.setJackpotWinning(win, jackpotCurrency);
            } else {
                //   bonuses and credits can only be integers
                //   Example: first player wins 1.1 bonuses, second player wins 1.1 bonuses, total 2.2. jackpot 2, the error will occur (payment),
                //   so rounded to the nearest whole (RoundingMode.DOWN), separately charged 1 bonus to each winning player (bypassing the patent)
                if (BigDecimal.valueOf(jackpot).setScale(2, RoundingMode.DOWN).compareTo(BigDecimal.valueOf(jackpot).setScale(0, RoundingMode.DOWN)) >= 1) {
                    //   ServiceCommunicatorImpl line 239
                    result.setAdditionalPaymentForWinning(true);
                }
                BigDecimal win = (BigDecimal.valueOf(jackpot).setScale(0, RoundingMode.UP));
                result.setJackpotWinning(win, jackpotCurrency);
            }
