...
//				.getServiceConnectionInformation(ProfileMessageTypes.SERVICE_NAME);
		RabbitClientSender.sendAsyncMessageWithClientInfo(ProfileMessageTypes.SERVICE_NAME, message, requestInfo);
	}
...
//					.getServiceConnectionInformation(UserMessageMessageTypes.SERVICE_NAME);
			...
			throw new F4MFatalErrorException("Unable to send push request to User Message Service", e);
		}
	}
...
//				.getServiceConnectionInformation(AuthMessageTypes.SERVICE_NAME);
		final RequestInfo requestInfo = new RequestInfoImpl(message);
		RabbitClientSender.sendAsyncMessageWithClientInfo(AuthMessageTypes.SERVICE_NAME, message, requestInfo);
	}
}
