...
describe('Testing direct communication', function() {
    ...
    it('test null token', function (done) {
        ...
        reqMessage.setMessage('test_service/ping');
    ...
    it('should receive authResponse', function (done) {
        var reqMessage = new ProtocolMessage();
        reqMessage.setMessage('global/auth');
        var sequenceNumber = serviceClient.getSeq();
        reqMessage.setSeq(sequenceNumber);
        reqMessage.setToken(token);
        ...
        serviceClient.sendMessage(reqMessage, function (err, resMessage) {
            try {
                ...
                assert.strictEqual(resMessage.getMessage(), reqMessage.getMessage() + 'Response');
                assert.strictEqual(resMessage.getError(), null);
                assert.deepEqual(resMessage.getMessageContainer(),{ message: 'global/authResponse', content: null, seq: null, ack: [ sequenceNumber ], error: null } );
                done();
            } catch (e) {
                done(e);
            }
    ...
    it('sending invalid json object', function (done) {
        serviceClient.on("message:errorResponse", function(resMessage) {
            try {
                assert.strictEqual(resMessage.getMessage(), 'global/errorResponse');
                assert.isNotNull(resMessage.getError());
                assert.deepEqual(resMessage.getMessageContainer(),{ message: "global/errorResponse", content: null, seq: null, ack: null, error: { type: Errors.ERR_VALIDATION_FAILED.type, message: Errors.ERR_VALIDATION_FAILED.message } } );
                done();
            } catch (e) {
                done(e);
            }
        ...
        serviceClient.connection.sendUTF("LOOLOLLLOLLTHAT CANT BE RIGH!!");
    });
