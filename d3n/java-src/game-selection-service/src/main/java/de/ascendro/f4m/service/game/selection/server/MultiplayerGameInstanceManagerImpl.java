...
public class MultiplayerGameInstanceManagerImpl implements MultiplayerGameInstanceManager {
	...
	public CustomGameConfig createMultiplayerGameInstance(MultiplayerGameParameters multiplayerGameParameters,
		...
		if (!GameUtil.isFreeGame(game, customGameConfig)) {
			paymentServiceCommunicator.sendCreateJackpotRequest(mgiId, customGameConfig, game, sourceMessage);
            if (game.getMinimumJackpotGarantie() != null && game.getMinimumJackpotGarantie() > 0) {
				...
				if (customGameConfig.getEntryFeeCurrency() == Currency.MONEY ||  !game.getResultConfiguration().isJackpotCalculateByEntryFee()) {
					paymentServiceCommunicator.sendTransferMinimumJackpotGuarantee(customGameConfig, game, sourceMessage, sourceSession, mgiId);
				}
            }
	...
	private ClientInfo buildClientInfo(ClientInfo clientInfo, String inviteeId) {
		if (clientInfo != null) {
			...
			inviteeClientInfo.setUserId(inviteeId);
	...
	@Override
	public void inviteUserIfNotExist(String userId, String instanceId) {
		if (mgiDao.getUserState(instanceId, userId) == null) {
			String inviterId = getGameCreatorId(instanceId);
			if (inviterId == null) {
				inviterId = userId;
			}
			mgiDao.addUser(instanceId, userId, inviterId, MultiplayerGameInstanceState.INVITED);
		}
	}
	...
	public void deleteDuelPrivate(String mgiId) {
		CustomGameConfig customGameConfig = mgiDao.getConfig(mgiId);
		if (!customGameConfig.isFree() && customGameConfig.getGameType().isDuel() && customGameConfig.getGameType().isMultiUser()) {
			mgiDao.deleteDuel(mgiId);
		}
	...
	public Invitation getInvitation(String tenantId, String appId, String userId, GameType gameType) {
		...
		List<Invitation> invitations = getInvitationList(tenantId, appId, userId,
				Arrays.asList(MultiplayerGameInstanceState.INVITED.name()), CreatedBy.OTHERS, filterCriteria);
