...
module.exports = {
    ...
    createProfile: function (message, clientSession) {
        if (!message.getMessage() || _.isNull(message.getSeq()) ||
            !_.has(message.getContent(), 'email') ||
            !_.has(message.getContent(), 'phone') ||
            !_.has(message.getContent(), 'facebook') ||
            !_.has(message.getContent(), 'google')) {
...
function _profileBlob(message) {
    ...
    if (message.getToken()) {
        var payload = jwt.decode(message.getToken(), { complete: true }).payload;
        profileBlob.value.userId = payload.userId;
    }
    return profileBlob;
}
