...
var FakeFriendService = require('./../services/frs.fakeService.js');
...
var WsHelper = function (series) {
    if (process.env.FAKE_SERVICES === 'true') {
        ...
        this._frFake = new FakeFriendService();
    }
...
WsHelper.prototype.build = function (done) {
    ...
    try {
        async.series([
            ...
            function (next) {
                if (process.env.FAKE_SERVICES === 'true') {
                    self._frFake.build(function (err) {
                        return next(err);
                    });
                } else {
                    return next();
        ...
        return done(Errors.MediaApi.FatalError);
    }
}
...
WsHelper.prototype.loadData = function (done) {
    var self = this;
    try {
        DataLoader.load(function (err, res) {
            if (err) {
                return done(err);
            }
            done();
