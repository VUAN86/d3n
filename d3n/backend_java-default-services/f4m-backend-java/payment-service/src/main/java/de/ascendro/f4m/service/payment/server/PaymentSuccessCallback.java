...
public class PaymentSuccessCallback extends HttpServlet {
	...
	private static final String TRANSACTION_ID = "token";
	...
	public PaymentSuccessCallback(TransactionLogCacheManager transactionLogCache, TransactionLogAerospikeDao transactionLogAerospikeDao,
								  ...
								  UserAccountManager userAccountManager, Tracker tracker, PaymentServiceCommunicator paymentServiceCommunicator,
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		LOGGER.info("Success callback received from payment system for transaction {}", transactionId);
		try {
	...
	public void onPaymentSuccess(String transactionId, boolean pushMessageToUser) {
		...
		if (logId != null) {
			...
			{
				convertToCredit(transactionLog);
				pushMessageToUser(transactionLog, transactionId, profileId, Messages.PAYMENT_SYSTEM_PAYMENT_SUCCESS_PUSH, WebsocketMessageType.PAYMENT_SUCCESS, appId);
			}
	...
	private void convertToCredit(TransactionLog transactionLog) {
		LOGGER.error("onPaymentSuccess transactionLog {} ", transactionLog);
		final String appId = transactionLog.getAppId();
		if (appId != null && appId.equals(paymentConfig.getProperty(PaymentConfig.PAYMENT_APP_ID))) {
			// we charge loans relatively exchangeRateList.
			final ExchangeRate exchangeRate = getExchangeRate(transactionLog);
			if (exchangeRate != null && exchangeRate.getFromAmount().equals(transactionLog.getAmount())) {
				if (exchangeRate.getFromCurrency().equals("EUR")) {
					...
					paymentServiceCommunicator.transferFundsToUserAccount(transactionLog.getTenantId(), transactionLog.getUserFromIdOrUserToId(), exchangeRate.getToAmount(),
																		  Currency.valueOf(exchangeRate.getToCurrency()), appId);
					// We charge for credits or bonuses.
					paymentServiceCommunicator.withdrawFromThePlayerAccount(transactionLog.getAppId(), transactionLog.getTenantId(), exchangeRate.getFromAmount(),
																			transactionLog.getUserFromId());
				}
			}
		}
	}
    ...
    private ExchangeRate getExchangeRate(TransactionLog transactionLog) {
        final List<ExchangeRate> exchangeRateList = currencyManager.getTenantExchangeRateList(transactionLog.getTenantId());
