...
var AchievementData = require('./achievement.data.js');
...
var TombolaData = require('./tombola.data.js');
var PromocodeData = require('./promocode.data.js');
    ...
    loadData: function (done) {
        Database.RdbmsService.sync({
            ...
            async.series([
                ...
                function (callback) { AchievementData.loadClassifiers(callback) },
                function (callback) { AdvertisementData.loadClassifiers(callback) },
                function (callback) { TenantData.loadClassifiers(callback) },
                // Load Entities
                ...
                function (callback) { TombolaData.loadEntities(callback) },
                function (callback) { PromocodeData.loadEntities(callback) },
                // Load Many-to-Many
                ...
                function (callback) { AchievementData.loadManyToMany(callback) },
                function (callback) { AdvertisementData.loadManyToMany(callback) },
                function (callback) { WorkorderSendMessageData.load(callback); },
                function (callback) { TombolaData.loadManyToMany(callback) },
                // Load Aerospike data, create secondary indexes
                function (callback) { ProfileData.loadAerospike(callback); },
            ], function (err) {
                if (err) {
                    if (process.env.DATABASE_LOGGER === 'true') {
