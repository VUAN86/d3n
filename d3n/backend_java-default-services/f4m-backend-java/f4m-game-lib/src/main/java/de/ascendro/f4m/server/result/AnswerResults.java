...
public class AnswerResults extends JsonObjectWrapper {
	...
	public void addOriginalOrderAnswers(String[] answers) {
		JsonObject infoJsonObject = getPropertyAsJsonObject(QUESTION_INFO_PROPERTY);
		if (infoJsonObject == null) infoJsonObject = new JsonObject();
		QuestionInfo question = new QuestionInfo(infoJsonObject);
		question.setPropertyOriginalOrderAnswers(answers);
	}
