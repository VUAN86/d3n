 ...
 */
public class WinningServiceServerMessageHandler extends JsonAuthenticationMessageMQHandler {
	...
	public WinningServiceServerMessageHandler(ResultEngineCommunicator resultEngineCommunicator,
			WinningComponentManager winningComponentManager, WinningManager winningManager,
			VoucherServiceCommunicator voucherServiceCommunicator, PaymentServiceCommunicator paymentServiceCommunicator,
			CommonProfileAerospikeDao profileDao, Tracker tracker, CommonGameInstanceAerospikeDao gameInstanceDao, GameAerospikeDao gameDao) {
	...
	private JsonMessageContent onWinningComponentGet(JsonMessage<WinningComponentGetRequest> message) {
		...
		winningComponent.setJackpotAmount(maxJackpotAmount);
	...
	private JsonMessageContent onWinningComponentList(JsonMessage<WinningComponentListRequest> message) {
		...
		final List<ApiWinningComponent> listItems = game.getWinningComponents() == null
				...
				: Arrays.stream(game.getWinningComponents())
						...
						.filter(winningComponent -> winningComponent != null)
	...
	private JsonMessageContent onUserWinningComponentAssign(JsonMessage<? extends UserWinningComponentAssignRequest> message) {
		...
		resultEngineCommunicator.requestGameInstanceResults(message, gameInstanceId);
		return null;
	}
	...
	private JsonMessageContent onUserWinningComponentStart(JsonMessage<UserWinningComponentStartRequest> message) {
		...
		if (winning != null) {
			if(winning.getType() == WinningOptionType.MONEY || winning.getType() == WinningOptionType.SUPER) {
				...
				paymentServiceCommunicator.requestTenantBalance(message, requestInfo);
				return null;
			}
	...
	private JsonMessageContent onUserWinningComponentStop(JsonMessage<UserWinningComponentStopRequest> message) {
		...
		if (userWinningComponent.getWinning().getType() == WinningOptionType.VOUCHER) {
			voucherServiceCommunicator.requestUserVoucherAssign(message, userWinningComponent,
					tenantId, appId);
		} else {
			paymentServiceCommunicator.requestWinningTransfer(message, userWinningComponent.getGameId(),
