...
import static de.ascendro.f4m.service.game.engine.config.GameEngineConfig.*;
...
public class QuestionSelector {
	...
	public List<Question> getAnswers(GameInstance gameInstance, String userId, QuestionPoolSelectionProperties selectionConfig, String userLanguage, String[] poolIds,
									 int numberOfQuestions, int skipsAvailable) throws F4MEntryNotFoundException {
//		final PoolSizesProvider poolSizesProvider = (questionTypesToSelect, questionComplexitiesToSelect) ->
//				questionPoolDao.getQuestionPoolSizes(poolIds, questionTypesToSelect, questionComplexitiesToSelect, userLanguage);
//		LOGGER.debug("poolSizesProvider {} ", poolSizesProvider);
//		final PoolEntryProvider<Question> poolEntryProvider = (baseKey, index) ->
//				questionPoolDao.getQuestion(new QuestionKey(baseKey, userLanguage), index);
//		LOGGER.debug("poolEntryProvider {} ", poolEntryProvider);
//		return selectQuestion(gameInstance, selectionConfig, numberOfQuestions, skipsAvailable, poolSizesProvider,
//				poolEntryProvider);\
        return null;
	}


	private <Q extends QuestionBaseKey> List<Q> selectQuestion(GameInstance gameInstance, QuestionPoolSelectionProperties selectionConfig, int numberOfQuestions, int skipsAvailable,
															   PoolSizesProvider poolSizesProvider, PoolEntryProvider<Q> poolEntryProvider) {
...
//		LOGGER.debug(" ");
//		LOGGER.debug("questionTypeUsage {}", questionTypeUsage.getText());
//		final String[] questionTypes = selectionConfig.getQuestionTypes();
//		LOGGER.debug("questionTypes {}", questionTypes);
//		final QuestionOverwriteUsage questionOverwriteUsage = selectionConfig.getQuestionOverwriteUsage();
//		LOGGER.debug("questionOverwriteUsage {}", questionOverwriteUsage.getText());
//
//		//Amount of questions
...
//		LOGGER.debug("fixedAmountOfQuestionsByType {}", fixedAmountOfQuestionsByType);
//
...
//		LOGGER.debug("fixedAmountOfQuestionsByComplexity {}", fixedAmountOfQuestionsByComplexity);
//
...
//				.toArray(String[]::new);
//		LOGGER.debug("questionTypesToSelect {}", questionTypesToSelect);
...
//				.toArray();
//		LOGGER.debug("questionComplexitiesToSelect {}", questionComplexitiesToSelect);
//
...
//		LOGGER.debug("questionPoolsSizes {}---{}", questionPoolsSizes.values(), questionPoolsSizes.keySet());
//
//		final List<Q> questions = new ArrayList<>(numberOfQuestions);
//		final Set<SelectedPoolEntry<Q>> selectedEntryIndex = new HashSet<>();
//		String[] ids = gameInstance.getQuestionIds();
...
//			LOGGER.debug("questionTypeIndex {}---complexityIndex {}", questionTypeIndex, complexityIndex);
//			LOGGER.debug("");
...
//			LOGGER.debug("type {}---complexity {}", type, complexity);
//
...
//							complexity);
//
//					final SelectedPoolEntry<Q> poolEntrys;
//
//					Q poolEntry = poolEntryProvider.getEntry(questionBaseKey, Long.parseLong(ids[q]));
//					if (poolEntry != null) {
//						poolEntrys = new SelectedPoolEntry<>(poolEntry, Long.parseLong(ids[q]));
//					} else {
//						throw new F4MQuestionCannotBeReadFromPool(
//								String.format("Failed to obtain question by key[%s] and random index after attempts",
//										questionBaseKey.toString()));
//					}
//
//					questions.add(poolEntrys.getEntry());
//					selectedEntryIndex.add(poolEntrys);
//					LOGGER.debug("poolIdAndSize {}---questionBaseKey {}--poolEntry {} ", poolIdAndSize, questionBaseKey, poolEntrys);
//				} else {
...
//		return questions;
		...
		for (int q = 0; q < numberOfQuestions; q++) {
			...
			if (sizeMeta.hasAnyQuestions()) {
				...
				if (poolIdAndSize.getValue() != null && poolIdAndSize.getValue() > 0) {
					...
					LOGGER.debug("poolIdAndSize {}---questionBaseKey {}--poolEntry {} ", poolIdAndSize, questionBaseKey,poolEntry);
				} else {
					throw new F4MQuestionsNotAvailableInPool(
							String.format("Selected question pool %s with complexity [%d] and type [%s] is of total size 0",
									poolIdAndSize.getKey(), sizeIdentifier.getCompelxity(),
	...
	private <Q extends QuestionBaseKey> SelectedPoolEntry<Q> readRandomPoolEntry(RandomSequenceGenerator randomSequenceGenerator,
		...
		do {// select question
			...
			LOGGER.debug("readRandomPoolEntry index [[{}]]  poolEntry [[{}]]", index, poolEntry);
			if (poolEntry == null) {
