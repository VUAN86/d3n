...
public class UserInteractionHandler {
	...
	private boolean onBonusPointTransfer(Results results, JsonMessage<? extends JsonMessageContent> request,
			SessionWrapper session) {
		...
		// Initiate payout
		serviceCommunicator.requestSinglePlayerGamePaymentTransfer(results.getUserId(), results.getGameId(), results.getMultiplayerGameInstanceId(),
				results.getGameInstanceId(), BigDecimal.valueOf(bonusPoints), Currency.BONUS, request, session);
		return true;
	}
