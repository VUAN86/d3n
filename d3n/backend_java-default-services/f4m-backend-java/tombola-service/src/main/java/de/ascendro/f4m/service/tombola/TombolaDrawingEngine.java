...
public class TombolaDrawingEngine {
	...
	public void tombolaDraw(String tombolaId, String tenantId, SessionWrapper sessionWrapper,
        ...
        if (purchasedTicketsNumber == 0) {
            ...
            releaseUnassignedVouchers(originalMessage, sessionWrapper, tombola, new ArrayList<>());
            ...
            payoutPrizes(originalMessage, sessionWrapper, tombola, tenantId, winners);

            releaseUnassignedVouchers(originalMessage, sessionWrapper, tombola, winners);
    ...
    private void releaseUnassignedVouchers(JsonMessage<? extends JsonMessageContent> originalMessage,
            SessionWrapper sessionWrapper, Tombola tombola, List<TombolaWinner> winners) {
    ...
    private void releaseUnassignedVouchersForPrize(Prize prize, List<TombolaWinner> winners,
            JsonMessage<? extends JsonMessageContent> sourceMessage, SessionWrapper sessionWrapper) {
        ...
        if (numberOfIntendedDraws > numberOfActualDraws) {
            ...
            IntStream.range(0, numberOfIntendedDraws - numberOfActualDraws).forEach(i ->
                    dependencyServicesCommunicator.requestUserVoucherRelease(sourceMessage, sessionWrapper,
                            voucherId));
        }
    }
    ...
    private void payoutPrizes(JsonMessage<? extends JsonMessageContent> originalMessage, SessionWrapper sessionWrapper,
            Tombola tombola, String tenantId, List<TombolaWinner> winners) {
        winners.forEach(tombolaWinner -> initiatePrizePayout(originalMessage, sessionWrapper, tombola, tenantId, tombolaWinner));
    }

    private void initiatePrizePayout(JsonMessage<? extends JsonMessageContent> sourceMessage,
            SessionWrapper sessionWrapper, Tombola tombola, String tenantId, TombolaWinner tombolaWinner) {
        ...
        // Send e-mail
        dependencyServicesCommunicator.sendTombolaEndToUser(tombolaWinner.getUser().getUserId(), tombola, tenantId, tombolaWinner.getAppId(),true);
