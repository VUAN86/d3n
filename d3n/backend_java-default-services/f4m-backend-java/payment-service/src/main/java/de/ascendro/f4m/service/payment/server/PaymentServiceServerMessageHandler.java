...
public class PaymentServiceServerMessageHandler extends DefaultJsonMessageHandler {
    ...
    public PaymentServiceServerMessageHandler(PaymentManager paymentManager, UserPaymentManager userPaymentManager, UserAccountManager userAccountManager,
                                              GameManager gameManager, AccountBalanceCache accountBalanceCache, AdminEmailForwarder adminEmailForwarder)
    {
	...
	public JsonMessageContent onUserMessage(RequestContext ctx) {
		...
		if (type != null) {
			...
			case INIT_EXTERNAL_PAYMENT:
				resultContent = userPaymentManager.initiateExternalPayment(getClientInfo(message), ((JsonMessage<InitExternalPaymentRequest>) message).getContent());
				break;
			case GET_EXTERNAL_PAYMENT:
				resultContent = userPaymentManager.getExternalPayment(getClientInfo(message), ((JsonMessage<GetExternalPaymentRequest>) message).getContent());
				break;
			case INIT_IDENTIFICATION:
				resultContent = userPaymentManager.initiateIdentification(getClientInfo(message), ((JsonMessage<InitIdentificationRequest>) message).getContent());
				break;
			case INSERT_OR_UPDATE_USER:
				resultContent = null;//
				userAccountManager.insertOrUpdateUser(((JsonMessage<InsertOrUpdateUserRequest>) message).getContent(), new JsonMessageHandlerCallback<>(this, ctx));
				break;
			case MERGE_USERS:
			...
			case CONVERT_BETWEEN_CURRENCIES:
				resultContent = paymentManager.convertBetweenCurrencies(getClientInfo(message),
						((JsonMessage<ConvertBetweenCurrenciesUserRequest>) message).getContent(),
						new JsonMessageHandlerCallback<>(this, ctx));
				break;
			...
			case GET_TRANSACTION:
				resultContent = paymentManager.getTransaction(((JsonMessage<GetTransactionRequest>) message).getContent(),
	...
	private JsonMessageContent getAccountBalance(JsonMessage<? extends JsonMessageContent> message, ClientInfo clientInfo) {
		...
		if (StringUtils.isNotEmpty(balanceRequest.getProfileId())) {
			resultContent = accountBalanceCache
					.getAccountBalance(message.getClientInfo(), balanceRequest);
		} else {
	...
	@Override
	public void onFailure(String originalMessageEncoded, RequestContext requestContext, Throwable e) {
