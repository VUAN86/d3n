...
public class ProfileAerospikeDaoImpl extends CommonProfileAerospikeDaoImpl implements ProfileAerospikeDao {
	...
	@Override
	public Profile updateProfile(String userId, final Profile profile, final Profile alreadyExisting, boolean isServiceResultEngine) {
		return updateProfile(userId, profile, alreadyExisting, false, isServiceResultEngine);
	}
	...
	private Profile updateProfile(String userId, final Profile profile, final Profile alreadyExisting, boolean overwrite, boolean isServiceResultEngine) {
		if (alreadyExisting != null) {
			...
			executeProfileChangeWithIdentifiersCleanup(userId, newIdentifiers, changedIdentifiers, () -> {
				...
				if (overwrite) {
					...
					updateJson(getSet(), primaryKey, BLOB_BIN_NAME, (res, policy) -> updateProfileJson(res, profile, isServiceResultEngine));
				}
			});
	...
	private String updateProfileJson(String profileFromDb, Profile profile, boolean isServiceResultEngine) {
		...
		Profile result = profileMerger.mergeProfileObjects(profile, dbProfile, isServiceResultEngine);
