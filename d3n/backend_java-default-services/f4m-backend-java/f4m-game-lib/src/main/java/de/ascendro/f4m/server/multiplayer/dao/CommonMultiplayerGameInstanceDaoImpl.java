...
public class CommonMultiplayerGameInstanceDaoImpl extends AerospikeOperateDaoImpl<MultiplayerGameInstancePrimaryKeyUtil>
	...
	private static final String GAME_INTANCE_ID_BIN_NAME = "gameInstanceId";
	private static final String COUNT_BIN_NAME = "count";
	@Inject
	...
	public void createDuel(String userId, CustomGameConfig customGameConfig, String mgiId) {
		final String metaKey = primaryKeyUtil.createMetaKey(mgiId);
		createOrUpdateRecord(DUEL_SET_NAME, metaKey,
	...
	public void updateDuel(String userId, CustomGameConfig customGameConfig, String mgiId, long decrement) {
		final String metaKey = primaryKeyUtil.createMetaKey(mgiId);
		createOrUpdateRecord(getIndexSet(), metaKey, getLongBin(CAPACITY_BIN_NAME, getRecordCapacity()),
	...
	public void addUser(String mgiId, String userId, String inviterId, MultiplayerGameInstanceState state) {
		// increase the number of attempts played.
		if (getIndexKeyToInstanceRecord(mgiId, userId) == null || numberOfAttempts(mgiId, userId) <= 3) {
			...
			createString(getIndexSet(), indexKey, INDEX_BIN_NAME, instanceKey, new WritePolicy());
			if (!getConfig(mgiId).getGameType().isTournament() || numberOfAttempts(mgiId, userId) <= 1) {
	...
	public void createOrUpdateTournamentLong(String mgiId, String userId, String binName, Long numberOfApproaches) {
		if (numberOfApproaches <= 3) {
			final String primaryKey = primaryKeyUtil.createIndexKeyToInstanceRecord(mgiId, userId);
			createOrUpdateLong(getIndexSet(), primaryKey, binName, numberOfApproaches);
		} else {
			throw new F4MGameInvitationNotValidException(
					String.format("User is not invited anymore (mgiId=[%s], userId=[%s])", mgiId, userId));
		}
	}

	public void recordUserHandicap(String mgiId, String userId, String userHandicap) {
		final String primaryKeyUtilIndexKeyToInstanceRecord = primaryKeyUtil.createIndexKeyToInstanceRecord(mgiId, userId);
		createOrUpdateString(getIndexSet(), primaryKeyUtilIndexKeyToInstanceRecord, "userHandicap", userHandicap);
	}

	public double getUserHandicap(String mgiId, String userId) {
		final String primaryKeyUtilIndexKeyToInstanceRecord = primaryKeyUtil.createIndexKeyToInstanceRecord(mgiId, userId);
		double handicap = 0.0;
		try {
			String handicapString = readString(getIndexSet(), primaryKeyUtilIndexKeyToInstanceRecord, "userHandicap");
			handicap = Double.parseDouble(handicapString != null ? handicapString : "0.0");
		} catch (NumberFormatException exception) {
			LOGGER.error("invalid data format. ", exception.getMessage());
		}
		return handicap;
	}

	public void createOrUpdateGameInstanceIdOfPreviousGame(String mgiId, String userId, String gameInstanceId) {
		final String primaryKeyUtilIndexKeyToInstanceRecord = primaryKeyUtil.createIndexKeyToInstanceRecord(mgiId, userId);
		createOrUpdateString(getIndexSet(), primaryKeyUtilIndexKeyToInstanceRecord, "gameInstanceId", gameInstanceId);
	}

	//   get the gameInstanceId previous attempt(TOURNAMENT).
	public String getGameInstanceOfThePreviousGame(String mgiId, String userId) {
		final String primaryKeyUtilIndexKeyToInstanceRecord = primaryKeyUtil.createIndexKeyToInstanceRecord(mgiId, userId);
		return readString(getIndexSet(), primaryKeyUtilIndexKeyToInstanceRecord, GAME_INTANCE_ID_BIN_NAME);
	}

	public Long numberOfAttempts(String mgiId, String userId) {
		final String primaryKey = primaryKeyUtil.createIndexKeyToInstanceRecord(mgiId, userId);
		Long count = readLong(getIndexSet(), primaryKey, COUNT_BIN_NAME);
		return count != null ? count : 0;
	}

	@Override
	public void removalOfThePreviousAttempt(String mgiId, ClientInfo clientInfo, String gameInstanceId) {
		saveAndGetMultiplayerUserGameInstance(mgiId, clientInfo, gameInstanceId, CALCULATED.getBinName(), STARTED.getBinName(), REGISTERED.getBinName(), CANCELLED.getBinName());
	}

	private void saveAndGetMultiplayerUserGameInstance(String mgiId, ClientInfo clientInfo, String gameInstanceId, String... bins) {
		final String indexKeyToInstanceRecord = getIndexKeyToInstanceRecord(mgiId, clientInfo.getUserId());
		Key key = new Key(getNamespace(), getSet(), indexKeyToInstanceRecord);
		Record record = getAerospikeClient().get(null, key, bins);
		if (record != null) {
			Stream.of(bins).forEach(binName -> {
                Map<?, ?> map = record.getMap(binName);
                if (map != null && map.get(gameInstanceId) != null) {
                    final MultiplayerUserGameInstance instance = jsonUtil.fromJson(String.valueOf(map.get(gameInstanceId)), MultiplayerUserGameInstance.class);
					saveRecordOfPreviousAttemptToPlayInTournament(mgiId, clientInfo.getUserId(), binName, instance, gameInstanceId);
				}
			});
		}
	}

	//   save the previous result of the game(TOURNAMENT).
	private void saveRecordOfPreviousAttemptToPlayInTournament(String mgiId, String userId, String binName, MultiplayerUserGameInstance multiplayerUserGameInstance, String gameInstanceId) {
		Long numberOfAttemptsUser = numberOfAttempts(mgiId, userId);
		final String primaryKey = primaryKeyUtil.createIndexKeyToInstance(mgiId, userId, --numberOfAttemptsUser);
		createJson(getIndexSet(), primaryKey, binName, jsonUtil.toJson(multiplayerUserGameInstance));
		deleteFromMap(mgiId, userId, gameInstanceId, binName);
	}

	//   delete the previous result of the game(TOURNAMENT).
	private void deleteFromMap(String mgiId, String userId, String gameInstanceId, String binName) {
		final String indexKeyToInstanceRecord = getIndexKeyToInstanceRecord(mgiId, userId);
		final Operation[] readOperations = createGetOperation(gameInstanceId, MapReturnType.VALUE, MultiplayerGameInstanceState.valueOf(binName.toUpperCase()));
		operate(getSet(), indexKeyToInstanceRecord, readOperations, (readResult, wp) -> {
			final Operation removeFromStateOperation = MapOperation.removeByKey(binName, Value.get(gameInstanceId), MapReturnType.NONE);
			return Collections.singletonList(removeFromStateOperation);
		});
	}
	...
	public CustomGameConfig getConfig(String mgiId) throws F4MEntryNotFoundException{
		...
		LOGGER.debug("getConfigAeroSpike key {}  set {}   ", key, getSet());
	...
	public void joinGame(String mgiId, String gameInstanceId, String userId, Double userHandicap) {
		final String indexKeyToInstanceRecord = getIndexKeyToInstanceRecord(mgiId, userId);
		LOGGER.debug("joinGame getSet() {} indexKeyToInstanceRecord {}", getSet(), indexKeyToInstanceRecord);
		...
		operate(getSet(), indexKeyToInstanceRecord, readOperations, (readResult, wp) -> {
			if (readResult == null || readResult.getValue(REGISTERED.getBinName()) == null) {
				...
				LOGGER.debug("joinGame multiplayerUserGameInstance ( {} )", multiplayerUserGameInstance);
	...
	@Override
	public boolean registerForGame(String mgiId, ClientInfo clientInfo, String gameInstanceId) {
		final String indexKeyToInstanceRecord = getIndexKeyToInstanceRecord(mgiId, clientInfo.getUserId());

		final Operation[] readOperations = createGetOperation(clientInfo.getUserId(), MapReturnType.VALUE, INVITED, REGISTERED);

		operate(getSet(), indexKeyToInstanceRecord, readOperations, (readResult, wp) -> {
			if (!hasValidInvitationState(readResult, INVITED) || numberOfAttempts(mgiId, clientInfo.getUserId()) > 3) {
	...
	private void changeInvitationStatus(String userId, String mgiId, MultiplayerGameInstanceState oldStatus,
        ...
        LOGGER.debug("changeInvitationStatus  gameConfig {}  tenantId{} appId{} invitationListKey{} getSet{}  {}  {}  userId{}  ",
                gameConfig, tenantId, appId, invitationListKey, getSet(), oldStatus, newStatus, userId);
		try {
			...
			LOGGER.debug("Error save status {}", e.getMessage());
		}
	}
	...
	private void addInstanceToInvitationList(String userId, String mgiId, String inviterId,
		...
		LOGGER.debug("addInstanceToInvitationList key {}  getSet {}  inviterId {}", key, getSet(), inviterId);
		createOrUpdateMapValueByKey(getSet(), key, status.getBinName(), mgiId, (Object readResult, WritePolicy wp) -> {
			if (readResult == null) {
				return inviterId;
			} else {
	...
	private void changeNotAcceptedInvitation(String mgiId, String userId, MultiplayerGameInstanceState newStatus) {
		...
		operate(getSet(), key, readOperations, (readResult, wp) -> {
			if (hasValidInvitationState(readResult, INVITED, DELETED)) {
				...
				return Collections.singletonList(MapOperation.put(MapPolicy.Default, INVITED.getBinName(), Value.get(userId), Value.get(newStatus.name())));
			} else {
	...
	private void invalidateExpiredInvitations(String tenantId, String appId, String userId) {
		...
		invitationsMap.keySet().forEach(mgiId -> {
	...
	public List<String> getExtendedMgiDuel() {
		...
		try (final RecordSet resultSet = getAerospikeClient().query(null, statement)) {
			while (resultSet.next()) {
				...
				LOGGER.debug("customGameConfig - {} ", customGameConfig);
	...
	private String getPK(String mgiId, String userId) {
		CustomGameConfig gameConfig = getConfig(mgiId);
		return primaryKeyUtil.createInvitationListKey(gameConfig.getTenantId(), gameConfig.getAppId(), userId);
	}
