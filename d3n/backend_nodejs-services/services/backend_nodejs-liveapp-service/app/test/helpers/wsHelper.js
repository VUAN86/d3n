...
var FakePaymentService = require('./../services/pay.fakeService.js');
...
var WsHelper = function (series) {
    if (process.env.FAKE_SERVICES === 'true') {
        this._tsFake = new FakePaymentService();
...
WsHelper.prototype.build = function (done) {
    ...
    try {
        async.series([
            function (next) {
                if (process.env.FAKE_SERVICES === 'true') {
                    self._tsFake.build(function (err) {
                        return next(err);
                    });
                } else {
                    return next();
...
WsHelper.prototype.initProfiles = function () {
    this._profiles = [ ProfileData.TEST_PROFILE ];
    this._profileBlobs = [];
}
