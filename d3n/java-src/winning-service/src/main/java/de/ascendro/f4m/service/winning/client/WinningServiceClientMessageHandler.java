...
public class WinningServiceClientMessageHandler extends JsonAuthenticationMessageMQHandler {
...
//	private final EventServiceClient eventServiceClient;
	...
	public WinningServiceClientMessageHandler(WinningComponentManager winningComponentManager,
			PaymentServiceCommunicator paymentServiceCommunicator, ResultEngineCommunicator resultEngineCommunicator,
			TransactionLogAerospikeDao transactionLogAerospikeDao, CommonUserWinningAerospikeDao userWinningDao, 
//			EventServiceClient eventServiceClient,
 			Tracker tracker,
			WinningManager winningManager, CommonProfileAerospikeDao profileDao, CommonGameInstanceAerospikeDao gameInstanceDao,
			UserMessageServiceCommunicator userMessageServiceCommunicator, SuperPrizeAerospikeDao superPrizeAerospikeDao) {
...
//	}
	...
	protected void handleUserErrorMessage(RequestContext ctx) {
		if(ctx.getOriginalRequestInfo() != null && ctx.getOriginalRequestInfo().getSourceMessage() != null &&
				ctx.getOriginalRequestInfo().getSourceMessage().getType(WinningMessageTypes.class) == WinningMessageTypes.USER_WINNING_COMPONENT_STOP &&
				ctx.getMessage().getError() != null && ctx.getMessage().getError().getCode().equals(PaymentServiceExceptionCodes.ERR_INSUFFICIENT_FUNDS)) {
    ...
    private void onGetResultsResponse(JsonMessage<GetResultsResponse> message, RequestInfo requestInfo) {
        ...
        switch (sourceMessageType) {
            case USER_WINNING_COMPONENT_ASSIGN:
                ...
                onWinningComponentAssignGotGetResultsResponse((JsonMessage<UserWinningComponentAssignRequest>) sourceMessage, results);
                break;
            default:
    ...
    private void onWinningComponentAssignGotGetResultsResponse(JsonMessage<UserWinningComponentAssignRequest> sourceMessage, Results results) {
        final String winningComponentId = getWinningComponentId(sourceMessage.getContent().getType(), results);
        final Game game = gameInstanceDao.getGameByInstanceId(sourceMessage.getContent().getGameInstanceId());
        ...
        if (component != null) {
            if (component.isPaid() && component.getAmount().signum() > 0) {
                paymentServiceCommunicator.requestAssignWinningComponentPayment(sourceMessage, results.getGameId(), sourceMessage.getContent().getGameInstanceId(), component, results.isEligibleToWinnings(),true);
            } else {
                onWinningComponentAssignGotTransferBetweenAccountsResponse(false, sourceMessage, winningComponentManager.getWinningComponent(sourceMessage.getClientInfo().getTenantId(),
						winningComponentId),
						component,
						results.isEligibleToWinnings(),
						true,
						null);
            }
        } else {
			component = game.getWinningComponent(sourceMessage.getContent().getWinningComponentId());
            if (component.isPaid())
                paymentServiceCommunicator.requestAssignWinningComponentPayment(sourceMessage, results.getGameId(), sourceMessage.getContent().getGameInstanceId(), component, results.isEligibleToWinnings(),false);
	...
	private String getWinningComponentId(WinningComponentType type, Results results) {
		final String winningComponentId;
		if (WinningComponentType.FREE == type) {
			winningComponentId = results.getFreeWinningComponentId();
		} else if (WinningComponentType.PAID == type) {
			winningComponentId = results.getPaidWinningComponentId();
		} else {
	...
	private void onTransferBetweenAccountsResponse(JsonMessage<TransactionId> message, PaymentRequestInfo requestInfo) {
		...
		transactionLogAerospikeDao.updateTransactionLog(requestInfo.getTransactionLogId(),
				transactionId.getTransactionId(), message.getError() == null ? TransactionStatus.COMPLETED : TransactionStatus.ERROR);
        ...
        switch (sourceMessageType) {
            case USER_WINNING_COMPONENT_ASSIGN:
                    ...
                    onWinningComponentAssignGotTransferBetweenAccountsResponse(true,
	...
	private void respondToAccountBalanceRequest(WinningComponentMoneyCheckRequestInfo requestInfo, BigDecimal amount) {
        ...
        WinningOption winning = winningComponent.getWinningOptions().stream().filter(o -> o.getWinningOptionId().equals(winningOptionId)).findFirst().get();
		...
		sendResponse(requestInfo.getSourceMessage(), new UserWinningComponentStartResponse(winning), requestInfo.getSourceMessageSource());
	}
    ...
    private void onWinningComponentAssignGotTransferBetweenAccountsResponse(boolean isPaidComponent,
																			JsonMessage<UserWinningComponentAssignRequest> sourceMessage,
																			WinningComponent winningComponent,
																			GameWinningComponentListItem winningComponentConfiguration,
																			boolean isEligibleToWinnings, boolean isEligibleToComponent,
																			JsonMessageError error) {
        if (error == null) {
            final UserWinningComponent userWinningComponent = isEligibleToWinnings && isEligibleToComponent
					? assignWinningComponentToUser(sourceMessage, winningComponent, winningComponentConfiguration)
					: null;
			...
			resultEngineCommunicator.storeUserWinningComponentInResults(sourceMessage, userWinningComponent);
		}
	}

    private UserWinningComponent assignWinningComponentToUser(JsonMessage<UserWinningComponentAssignRequest> sourceMessage, WinningComponent winningComponent, GameWinningComponentListItem winningComponentConfiguration) {
        return winningComponentManager.assignWinningComponentToUser(sourceMessage.getClientInfo().getTenantId(), sourceMessage.getClientInfo().getAppId(), sourceMessage.getClientInfo().getUserId(), sourceMessage.getContent().getGameInstanceId(), winningComponent, winningComponentConfiguration);
    }
    ...
    private void onUserWinningComponentAssignGotStoreWinningComponentResponse(UserWinningComponentRequestInfo requestInfo) {
        ...
        sendResponse(requestInfo.getSourceMessage(), response, requestInfo.getSourceMessageSource());
    }
    ...
    private void sendResponseToWinningComponentStop(UserWinningComponentRequestInfo requestInfo) {
        ...
        sendResponse(requestInfo.getSourceMessage(), new UserWinningComponentStopRequest(requestInfo.getUserWinningComponent().getUserWinningComponentId()), requestInfo.getSourceMessageSource());
    }
