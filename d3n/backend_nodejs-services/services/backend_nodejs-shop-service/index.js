...
var ShopService = function (config) {
    var self = this;
...
ShopService.prototype.build = function (callback) {
    var self = this;
    if (!self._built) {
...
ShopService.prototype.shutdown = function (callback) {
    var self = this;
...
var argp = yargs
    ...
    .options({
        build: {
            boolean: true,
            describe: 'Build Shop Template Factory service.'
        }
    }
);
var argv = argp.argv;
...
if (argv.build === true) {
    var service = new ShopService(Config);
    service.build(function (err) {
        if (err) {
            logger.error('Error building Shop Factory service: ', err);
            process.exit(1);
        }
        logger.info('Shop Factory service started successfully');
    });
}