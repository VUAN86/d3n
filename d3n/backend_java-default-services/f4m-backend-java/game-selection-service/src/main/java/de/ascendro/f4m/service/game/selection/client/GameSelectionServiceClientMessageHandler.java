...
public class GameSelectionServiceClientMessageHandler extends DefaultJsonMessageHandler {
	...
	private void onCreateJackpotResponse(PaymentCreateJackpotRequestInfo originalRequestInfo) {
		if (originalRequestInfo.getSourceMessage() != null) {
			...
			mgiManager.sendTransferMinimumJackpotGuarantee(originalRequestInfo.getMgiId());
	...
	private void fillEntryWithJackpot(PaymentGetJackpotRequestInfo originalRequestInfo,
		...
		if (originalRequestInfo.getResponseToForward() instanceof GetGameListResponse) {
			...
			countingTheJackpot(invitationToChange, message);
	...
	public void countingTheJackpot(Invitation invitationToChange, JsonMessage<GetJackpotResponse> message) {
		LOGGER.debug("countingTheJackpot minimumJackpotGarantie {} ", invitationToChange.getMinimumJackpotGarantie());
		LOGGER.debug("countingTheJackpot jackpot {} ", message.getContent().getBalance());
		if (invitationToChange.getGame().getType().isTournament() && invitationToChange.getMinimumJackpotGarantie() != null
				&& message.getContent().getBalance() != null && message.getContent().getCurrency() == Currency.MONEY)
		{
			BigDecimal jackpotMinusMinimumJackpotGuarantee = message.getContent().getBalance().subtract(BigDecimal.valueOf(invitationToChange.getMinimumJackpotGarantie()));
			if (jackpotMinusMinimumJackpotGuarantee.compareTo(BigDecimal.valueOf(invitationToChange.getMinimumJackpotGarantie())) > 0) {
				BigDecimal jackpotMinusPayoutPercent = jackpotMinusMinimumJackpotGuarantee.multiply(config.getPropertyAsBigDecimal(GameSelectionConfig.JACKPOT_PAYOUT_PERCENT));
				invitationToChange.getGame().setJackpot(new Jackpot(jackpotMinusPayoutPercent, Currency.MONEY));
			} else {
				invitationToChange.getGame().setJackpot(new Jackpot(BigDecimal.valueOf(invitationToChange.getMinimumJackpotGarantie()), Currency.MONEY));
			}
		} else {
			invitationToChange.getGame().setJackpot(new Jackpot(message.getContent().getBalance(), message.getContent().getCurrency()));
		}
	}
	...
	private void onNotifySubscriber(NotifySubscriberMessageContent notificationMessage) {
		...
		LOGGER.debug("onNotifySubscriber topic {}  , subscr {} , content {}", topic, notificationMessage.getSubscription(), notificationMessage.getNotificationContent());
