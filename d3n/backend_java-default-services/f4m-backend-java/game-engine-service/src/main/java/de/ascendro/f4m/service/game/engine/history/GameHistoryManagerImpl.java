...
import de.ascendro.f4m.service.game.engine.exception.F4MErrorEndGame;
...
public class GameHistoryManagerImpl implements GameHistoryManager {
	...
	/**
	 *  The end of the duel, if anyone of the players flew either in a duel played only single player.
	 *
	 * @see ActiveGameTimer  TEN_MINUTES_IN_MILLIS    (the timer runs every 10 minutes)
	 */
	private void processExpiredDuel() {
		LOGGER.debug("GameHistoryManagerImpl.processExpiredDuel");
        for (String mgiId : commonMultiplayerGameInstanceDao.getExtendedMgiDuel()) {

            try {
                List<MultiplayerUserGameInstance> multiplayerUserGameInstances = commonMultiplayerGameInstanceDao.getGameInstances(mgiId);

				LOGGER.debug("GameHistoryManagerImpl.processExpiredDuel mult - {} ", multiplayerUserGameInstances);
                if (!isOnlyInviterRegistered(mgiId) && commonMultiplayerGameInstanceDao.getNotYetCalculatedGameInstancesCount(mgiId) == 1) {
					...
					calculationOfResults(getGameInstanceId(multiplayerUserGameInstances));
					...
					calculationOfResults(multiplayerUserGameInstances.get(0).getGameInstanceId());
					calculationOfResults(multiplayerUserGameInstances.get(1).getGameInstanceId());
	...
	private void calculationOfResults(String gameInstanceId) {
		GameInstance gameInstance = gameInstanceAerospikeDao.calculateIfUnfinished(gameInstanceId, CloseUpReason.EXPIRED, null, null, true);
		resultEngineCommunicator.requestCalculateResults(null, gameInstance, null);
	}
	...
	private void processExpiredTournament() {
		...
		for (String mgiId : mgiIds) {
			...
			try {
				GameInstance gameInstance = calculateIfUnfinished(commonMultiplayerGameInstanceDao.getGameInstances(mgiId));
				requestCalculateMultiplayerResultsIfPossible(mgiId, gameInstance);
	...
	private GameInstance calculateIfUnfinished(List<MultiplayerUserGameInstance> gameInstances) {
		GameInstance gameInstance = null;
		for (MultiplayerUserGameInstance multiplayerUserGameInstance : gameInstances) {
			gameInstance = gameInstanceAerospikeDao.calculateIfUnfinished
					(multiplayerUserGameInstance.getGameInstanceId(), CloseUpReason.EXPIRED, null, null, true);
		}
		return gameInstance;
	}

	private void requestCalculateMultiplayerResultsIfPossible(String mgiId, GameInstance gameInstance) {
		if (gameInstance != null) {
			LOGGER.debug("requestCalculateMultiplayerResultsIfPossible 1 gameInstance {} ", gameInstance);
			multiplayerGameManager.requestCalculateMultiplayerResultsIfPossible(buildClientInfo(gameInstance), gameInstance.getMgiId(), gameInstance.getGame(), null);
		} else {
			CustomGameConfig config = commonMultiplayerGameInstanceDao.getConfig(mgiId);
			if (config.getGameType().isTournament() && !config.isFree()) {
				paymentServiceCommunicator.sendGetJackpotReq(mgiId, config, null, null);
			}
			throw new F4MErrorEndGame("Error at the end of the game.");
		}
	}
	...
	@Override
	public void processPerformGameEndTippTournament(String mgiId) {
		paymentServiceCommunicator.refundPayment(mgiId, commonMultiplayerGameInstanceDao.getConfig(mgiId));
	}
	...
	@Override
	public boolean isOnlyInviterRegistered(String mgiId) {
