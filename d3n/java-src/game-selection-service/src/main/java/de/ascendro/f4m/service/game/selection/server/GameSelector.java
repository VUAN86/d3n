...
public class GameSelector {
	...
	public GetGameListResponse getGameList(String userId, GameFilter gameFilter) {
		...
		gameList = removeExcessInfo(gameList);
	...
	/**
	 * Filters out special games that have exceeded play count
	 * */
	...
	protected JsonArray sortBySpecialGameWeight(JsonArray gameList) {
		...
		for (JsonElement gameElement : gameList) {
			games.add(new ImmutablePair<Game, JsonElement>(jsonUtil.fromJson(gameElement, Game.class), gameElement));
		}
		return games.stream()
